...
def _fpp(count_dict, total_dict, thresh):
    dict_ratio = dict()
    for w in count_dict.keys():
...
class SamplingAndOcclusionExplain:
    def __init__(self, model, configs, tokenizer, processor, output_path, device, lm_dir=None):
        logger.info('Current version of SOC: 0.000.301')
    ...
    def _update_suppress_words(self):
        ...
        for w_ids in word_counts_dict.keys():
            ...
            if word_counts_dict[w_ids] >= self.window_count:
                # TODO: 0. remove words when it reaches thresh
    ...
    def suppress_explanation_loss(self, input_ids_batch, input_mask_batch, segment_ids_batch, label_ids_batch, do_backprop=False):
